...
flight animations, and one representative waypoint summary for **CYVR -> KCLT**.

Scenario ID from COBALT: `bb90ec5a-9a28-4e65-a352-65fe2d1dfad4`  
...
- `data/bb90ec5a-9a28-4e65-a352-65fe2d1dfad4_multicon_science.nc` — CF-1.11 NetCDF scientific dataset (222 waypoints).
- `animations/` — IWC, number concentration, optical depth, signed settling
  velocity, effective radius, and shape-index GIFs.
- `snapshot/row_0074_scientific_summary.png` — row 74 summary.
...
| `row_time` | `seconds` | `row` | UTC time at which the waypoint plume is initialized. |
...
```python
import xarray as xr
ds = xr.open_dataset("data/bb90ec5a-9a28-4e65-a352-65fe2d1dfad4_multicon_science.nc", engine="h5netcdf")
print(ds)
```

MATLAB: `ncdisp('data/bb90ec5a-9a28-4e65-a352-65fe2d1dfad4_multicon_science.nc')`  
R: `ncdf4::nc_open('data/bb90ec5a-9a28-4e65-a352-65fe2d1dfad4_multicon_science.nc')`
...
Verify transfer integrity with `checksums.sha256` before use.
